 ...
 * Управляет камерой в сочетании с базовым
 * {@link Detector}.  This receives preview frames from the camera at
 * Это получает кадры предварительного просмотра с камеры по адресу
 * a specified rate, sending those frames to the detector as fast as it is able to process those
 * frames.
 * заданную скорость, отправляя эти кадры в детектор так быстро, как он способен обрабатывать эти кадры
 * <p/>
